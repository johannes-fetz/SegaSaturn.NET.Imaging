...
namespace Paloma
{
    ...
    public class TargaImage : IDisposable
    {
        ...
        private byte[] LoadImageBytes(BinaryReader binReader)
        {
            ...
            if (binReader != null && binReader.BaseStream != null && binReader.BaseStream.Length > 0 && binReader.BaseStream.CanSeek)
            {
                if (this.objTargaHeader.ImageDataOffset > 0)
                {
                    ...
                    int intImageRowByteSize = this.objTargaHeader.Width * this.objTargaHeader.BytesPerPixel;

                    // get the size in bytes of the whole image
                    int intImageByteSize = intImageRowByteSize * this.objTargaHeader.Height;
                       ...
                       this.objTargaHeader.ImageType == ImageType.RUN_LENGTH_ENCODED_TRUE_COLOR)
                    {
                        ...
                        while (intImageBytesRead < intImageByteSize)
                        {
                            ...
                            if ((RLEPacketType)intRLEPacketType == RLEPacketType.RUN_LENGTH)
                            {
                                ...
                                bRunLengthPixel = binReader.ReadBytes(this.objTargaHeader.BytesPerPixel);
        ...
        private void LoadTGAImage(BinaryReader binReader)
        {
            ...
            this.intStride = ((this.objTargaHeader.Width * this.objTargaHeader.PixelDepth + 31) & ~31) >> 3; // width in bytes
    ...
    public class TargaHeader
    {
        ...
        public int ImageDataOffset
        {
            get
            {
                ...
                intImageDataOffset += (this.sColorMapLength * Bytes);
    ...
    public class TargaExtensionArea
    {
        ...
        int intPixelAspectRatioNumerator;
        int intPixelAspectRatioDenominator;
        int intGammaNumerator;
        int intGammaDenominator;
        int intColorCorrectionOffset;
        int intPostageStampOffset;
        int intScanLineOffset;
        int intAttributesType;
        ...
        public float PixelAspectRatio
        {
            get
            {
                ...
                {
                    return this.intPixelAspectRatioNumerator / (float)this.intPixelAspectRatioDenominator;
                }
                return 0.0F;
            }
        }
        ...
        public float GammaRatio
        {
            get
            {
                ...
                {
                    float ratio = this.intGammaNumerator / (float)this.intGammaDenominator;
